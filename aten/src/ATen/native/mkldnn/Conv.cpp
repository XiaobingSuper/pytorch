...
ideep::tensor _mkldnn_convolution_transpose(
  ...
  std::vector<int64_t> kernel_size(x.ndims());
  // mkldnn convolution transpose weights could not have been re-ordered to 5d by
  // mkldnn_reorder_conv2d_weight
  if (w.ndims() == x.ndims() + 1) {
    AT_ASSERTM(
        groups > 1,
  ...
  ideep::tensor y;
  if (b.has_value()) {
    ideep::convolution_transpose_forward::compute<AllocForMKLDNN>(
        x,
        w,
        b.value(),
        {output_sizes.cbegin(), output_sizes.cend()},
...
ideep::tensor _mkldnn_convolution_transpose_backward_input(
  ...
  ideep::tensor gradx;
  ideep::convolution_transpose_backward_data::compute<AllocForMKLDNN>(
      grady,
      w,
      {input_sizes.cbegin(), input_sizes.cend()},
      gradx,
...
std::tuple<ideep::tensor, ideep::tensor> _mkldnn_convolution_transpose_backward_weights(
  ...
  ideep::tensor gradw, gradb;
  if (bias_defined) {
    ideep::convolution_transpose_backward_weights::compute<AllocForMKLDNN>(
        x,
        grady,
        {weight_sizes.cbegin(), weight_sizes.cend()},
        gradw,
...
Tensor mkldnn_convolution_transpose(
    ...
    IntArrayRef dilation,
    int64_t groups) {
  auto weight_ = weight.transpose(0,1).contiguous();
  const ideep::tensor mkldnn_input = get_mkldnn_tensor(input);
  const ideep::tensor mkldnn_weight = get_mkldnn_tensor(weight_);
  c10::optional<ideep::tensor> mkldnn_bias{c10::nullopt};
  if (bias.defined()) {
    mkldnn_bias = get_mkldnn_tensor(bias);
  }
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_convolution_transpose_backward(const Tensor& input,
  ...
  if (output_mask[1] || output_mask[2]) {
    std::tie(grad_weight, grad_bias) = at::mkldnn_convolution_transpose_backward_weights(
      weight, grad_output, input, padding, output_padding, stride, dilation, groups, output_mask[2]);
  }
  auto grad_weight_t = grad_weight.transpose(0 , 1);
  return std::tuple<Tensor, Tensor, Tensor>{grad_input, grad_weight_t, grad_bias};
}
