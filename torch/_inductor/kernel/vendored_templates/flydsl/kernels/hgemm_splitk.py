...
def compile_hgemm_kernel(
    ...
    B_TO_LDS: bool = False,
    HAS_BIAS: bool = False,
    # Optional fused epilogue support.  When enabled, the kernel calls
    # EPILOGUE_FN(acc, *epilogue_args) before storing C.  `acc` is the f32
    # accumulator vector; the return value is cast back to the output dtype.
    HAS_EPILOGUE: bool = False,
    # FlyDSL @flyc.jit callable generated by Inductor/FlexGEMM.  It receives
    # the accumulator plus any captured epilogue tensors loaded according to
    # EPILOGUE_ARG_KINDS.
    EPILOGUE_FN=None,
    # Broadcast layout for each captured epilogue tensor.  Currently supported:
    # "row" -> shape [N] or [1, N], loaded along the output N dimension.
    # "tile" -> shape [M, N], loaded elementwise with the output tile.
    EPILOGUE_ARG_KINDS: tuple[str, ...] = (),
    # FlyDSL dtype string for each captured epilogue tensor ("f16", "bf16",
    # "f32", or "i8" for bool storage).  Must align 1:1 with ARG_KINDS.
    EPILOGUE_ARG_DTYPES: tuple[str, ...] = (),
    # True when EPILOGUE_FN returns (main_output, aux_output).  The main output
    # is stored to C; aux_output is stored to the AUX_OUT pointer.
    RETURNS_AUX: bool = False,
    # FlyDSL dtype string for the aux output buffer when RETURNS_AUX=True.
    AUX_OUT_DTYPE: str = "f16",
):
    ...
    if B_TO_LDS:
        ...
        assert ASYNC_COPY
    C_STAGING_BYTES = 4 if HAS_EPILOGUE else DTYPE_BYTES
    SMEM_USE_ = max(SMEM_USE, BLOCK_K_WARPS * BLOCK_M * BLOCK_N * C_STAGING_BYTES)
    ...
    if HAS_BIAS:
        KERNEL_NAME += "_BIAS"
    if HAS_EPILOGUE:
        assert not IS_SPLIT_K, "FlyDSL hgemm epilogue fusion requires SPLIT_K == 1"
        if EPILOGUE_FN is None:
            raise AssertionError("EPILOGUE_FN must be set when HAS_EPILOGUE=True")
        if len(EPILOGUE_ARG_KINDS) != len(EPILOGUE_ARG_DTYPES):
            raise AssertionError(
                "EPILOGUE_ARG_KINDS and EPILOGUE_ARG_DTYPES length mismatch"
            )
        if len(EPILOGUE_ARG_KINDS) > 4:
            raise AssertionError("FlyDSL hgemm supports at most 4 epilogue args")
        for kind in EPILOGUE_ARG_KINDS:
            if kind not in ("row", "tile"):
                raise NotImplementedError(
                    f"FlyDSL hgemm epilogue arg kind {kind!r} is not supported yet"
                )
        if RETURNS_AUX and AUX_OUT_DTYPE not in ("f16", "bf16", "f32", "i8"):
            raise NotImplementedError(
                f"FlyDSL hgemm aux output dtype {AUX_OUT_DTYPE!r} is not supported"
            )
        KERNEL_NAME += "_EPILOGUE"
    if RETURNS_AUX:
        if not HAS_EPILOGUE:
            raise AssertionError("RETURNS_AUX requires HAS_EPILOGUE")
        KERNEL_NAME += "_AUX"
    ...
    def hgemm_kernel(
        ...
        c_staging_dtype_ = T.f32 if const_expr(HAS_EPILOGUE) else dtype_
        ...
        if const_expr(RETURNS_AUX):
            AUX_OUT_ = GTensor(
                AUX_OUT, dtype=get_dtype_in_kernel(AUX_OUT_DTYPE), shape=(-1, n)
            )
        if const_expr(HAS_BIAS):
            BIAS_ = GTensor(BIAS, dtype=dtype_, shape=(n,))
        if const_expr(len(EPILOGUE_ARG_KINDS) > 0):
            AUX0_ = GTensor(
                AUX0,
                dtype=get_dtype_in_kernel(EPILOGUE_ARG_DTYPES[0]),
                shape=(n,) if const_expr(EPILOGUE_ARG_KINDS[0] == "row") else (-1, n),
            )
        if const_expr(len(EPILOGUE_ARG_KINDS) > 1):
            AUX1_ = GTensor(
                AUX1,
                dtype=get_dtype_in_kernel(EPILOGUE_ARG_DTYPES[1]),
                shape=(n,) if const_expr(EPILOGUE_ARG_KINDS[1] == "row") else (-1, n),
            )
        if const_expr(len(EPILOGUE_ARG_KINDS) > 2):
            AUX2_ = GTensor(
                AUX2,
                dtype=get_dtype_in_kernel(EPILOGUE_ARG_DTYPES[2]),
                shape=(n,) if const_expr(EPILOGUE_ARG_KINDS[2] == "row") else (-1, n),
            )
        if const_expr(len(EPILOGUE_ARG_KINDS) > 3):
            AUX3_ = GTensor(
                AUX3,
                dtype=get_dtype_in_kernel(EPILOGUE_ARG_DTYPES[3]),
                shape=(n,) if const_expr(EPILOGUE_ARG_KINDS[3] == "row") else (-1, n),
            )
        ...
        cs_ = STensor(
            smem_c_ptr, c_staging_dtype_, shape=(BLOCK_K_WARPS, BLOCK_M, BLOCK_N)
        )
        ...
        def load_epilogue_arg(aux_tensor, kind):
            if const_expr(kind == "row"):
                return aux_tensor.vec_load((n_offset + n_local_idx,), LDG_VEC_SIZE)
            if const_expr(kind == "tile"):
                return aux_tensor.vec_load(
                    (m_global_idx, n_offset + n_local_idx), LDG_VEC_SIZE
                )
            raise NotImplementedError(f"Unsupported epilogue arg kind: {kind}")

        def load_epilogue_args():
            args = []
            if const_expr(len(EPILOGUE_ARG_KINDS) > 0):
                args.append(load_epilogue_arg(AUX0_, EPILOGUE_ARG_KINDS[0]))
            if const_expr(len(EPILOGUE_ARG_KINDS) > 1):
                args.append(load_epilogue_arg(AUX1_, EPILOGUE_ARG_KINDS[1]))
            if const_expr(len(EPILOGUE_ARG_KINDS) > 2):
                args.append(load_epilogue_arg(AUX2_, EPILOGUE_ARG_KINDS[2]))
            if const_expr(len(EPILOGUE_ARG_KINDS) > 3):
                args.append(load_epilogue_arg(AUX3_, EPILOGUE_ARG_KINDS[3]))
            return tuple(args)
        ...
        for ii in range_constexpr(WARP_M_STEPS):
            ...
            for jj in range_constexpr(WARP_N_STEPS):
                ...
                for kk in range_constexpr(WMMA_C_FRAG_VALUES):
                    ...
                    if const_expr(not HAS_EPILOGUE):
            ...
            for i in range_constexpr(LDG_REG_C_COUNT):
                ...
                with ir.InsertionPoint(cond_boundary_if.then_block):
                    ...
                    if const_expr(HAS_BIAS):
                        ...
                        if const_expr(HAS_EPILOGUE):
                            bias_vec = bias_vec.to(fx.Float32)
                        vec = vec + bias_vec
                    if const_expr(HAS_EPILOGUE):
                        if const_expr(RETURNS_AUX):
                            vec, aux_vec = EPILOGUE_FN(
                                vec, *load_epilogue_args()
                            )
                            if const_expr(AUX_OUT_DTYPE == "bf16"):
                                aux_vec = aux_vec.to(fx.BFloat16)
                            elif const_expr(AUX_OUT_DTYPE == "f16"):
                                aux_vec = aux_vec.to(fx.Float16)
                            elif const_expr(AUX_OUT_DTYPE == "i8"):
                                aux_vec = aux_vec.to(fx.Uint8)
                            AUX_OUT_.vec_store(
                            ...
                            vec = EPILOGUE_FN(vec, *load_epilogue_args())
                        if const_expr(dtype == "bf16"):
                            vec = vec.to(fx.BFloat16)
                        else:
                            vec = vec.to(fx.Float16)
                    C_.vec_store(
    ...
    def launch_hgemm_kernel(
        ...
        hgemm_kernel(
            C, A, B, BIAS, m, semaphore, signal, AUX0, AUX1, AUX2, AUX3, AUX_OUT
        ).launch(
