...
def compile_hgemm_kernel(
    ...
    if GROUPED and IS_SPLIT_K:
        raise NotImplementedError("Grouped hgemm does not support SPLIT_K yet")
    ...
    if HAS_BIAS:
        KERNEL_NAME += "_BIAS"
    if GROUPED:
        KERNEL_NAME += f"_GROUPED{GROUP_COUNT}"
        if GROUPED_TILE_MAP:
            KERNEL_NAME += "_TILEMAP"
    ...
    def hgemm_kernel(
        ...
        if const_expr(GROUPED):
            B_ = GTensor(B, dtype=dtype_, shape=(GROUP_COUNT, k, n))
            OFFS_ = GTensor(OFFS, dtype=T.i32, shape=(GROUP_COUNT,))
            if const_expr(GROUPED_TILE_MAP):
        ...
        if const_expr(GROUPED):
            block_m_idx = fx.Index(0)
            block_n_idx = fx.Index(0)
            group_idx = fx.Index(0)
            group_start = fx.Index(0)
            row_limit = fx.Index(0)
        else:
            block_m_idx, block_n_idx = swizzle_for_cache_reuse(fx.block_idx.x)
            group_idx = fx.Index(0)
            group_start = fx.Index(0)
            row_limit = fx.Index(m)
        ks_idx = fx.Index(0) if const_expr(GROUPED) else fx.Index(fx.block_idx.y)
        ks_begin = arith.index_cast(T.i32, ks_idx * ks)

        m_offset = group_start + fx.Index(block_m_idx * BLOCK_M)
        ...
        def __barrier(vmcnt=0, use_s_barrier=True):
            if const_expr(use_s_barrier):
                if const_expr(GROUPED and B_TO_LDS):
                    asm = f"s_waitcnt vmcnt({vmcnt}) lgkmcnt(0)\n\ts_barrier"
                else:
            ...
            with ir.InsertionPoint(cond_ks0_if.then_block):
                ...
                for i in range_constexpr(LDG_REG_C_COUNT):
                    ...
                    cond_boundary = arith.cmpi(
                        arith.CmpIPredicate.ult, row_idx, row_limit
                    )
        ...
        def ldg_a(k_offset):
            ...
            for i in range_constexpr(LDG_REG_A_COUNT):
                ...
                safe_row_idx = arith.select(
                    arith.cmpi(arith.CmpIPredicate.ult, row_idx, row_limit),
                    row_idx,
                    fx.Index(0),
                )
        ...
        def ldg_sts_b_async_one(ii, k_offset, write_stage, lds_ptr=None):
            ...
            if const_expr(GROUPED):
                vec = vector.from_elements(
                    T.vec(LDG_ASYNC_VEC_SIZE, dtype_),
                    [
                        B_[group_idx, fx.Index(col_idx + bi), safe_row_idx]
                        for bi in range_constexpr(LDG_ASYNC_VEC_SIZE)
                    ],
                )
                bs_.vec_store(
                    (
                        fx.Index(write_stage),
                        n_local_idx,
            ...
            if const_expr(GROUPED):
                global_offset = (
                    B_.linear_offset((group_idx, col_idx, safe_row_idx)) * DTYPE_BYTES
                )
            else:
        ...
        def ldg_matrix_b(k_offset):
            ...
            for kk in range_constexpr(WARP_K_STEPS):
                for ii in range_constexpr(WARP_N_STEPS):
                    ...
                    if const_expr(GROUPED):
                        vec = vector.from_elements(
                            T.vec(WMMA_B_FRAG_VALUES * MFMA_PER_WARP_K, dtype_),
                            [
                                B_[group_idx, fx.Index(k_idx + bi), n_idx]
                                for bi in range_constexpr(
        ...
        def compute_one_tile():
            ...
            if const_expr(IS_SPLIT_K):
                ...
                for i in range_constexpr(LDG_REG_C_COUNT):
                    ...
                    cond_boundary = arith.cmpi(
                        arith.CmpIPredicate.ult, m_global_idx, row_limit
                    )
        ...
        if const_expr(GROUPED):
            tile_id = fx.Index(fx.block_idx.x)
            if const_expr(GROUPED_TILE_MAP):
                tile_valid = arith.cmpi(
                    arith.CmpIPredicate.ult, tile_id, fx.Index(TOTAL_TILES_[0])
                )
                tile_valid_if = scf.IfOp(tile_valid, results_=[], has_else=False)
                with ir.InsertionPoint(tile_valid_if.then_block):
                    group_idx = fx.Index(TILE_GROUPS_[tile_id])
                    block_m_idx = fx.Index(TILE_MS_[tile_id])
                    block_n_idx = fx.Index(TILE_NS_[tile_id])
                    has_prev_group = arith.cmpi(
                        arith.CmpIPredicate.ugt, group_idx, fx.Index(0)
                    )
                    prev_group = arith.select(has_prev_group, group_idx - 1, fx.Index(0))
                    group_start = arith.select(
                        has_prev_group, fx.Index(OFFS_[prev_group]), fx.Index(0)
                    )
                    row_limit = fx.Index(OFFS_[group_idx])
                    m_offset = group_start + fx.Index(block_m_idx * BLOCK_M)
                    n_offset = fx.Index(block_n_idx * BLOCK_N)
                    compute_one_tile()
                    scf.YieldOp([])
            else:
                block_n_idx = tile_id % N_BLOCKS
                tile_m_group = tile_id // N_BLOCKS
                block_m_idx = tile_m_group % fx.Index(max_m_tiles)
                group_idx = tile_m_group // fx.Index(max_m_tiles)
                has_prev_group = arith.cmpi(
                    arith.CmpIPredicate.ugt, group_idx, fx.Index(0)
                )
                prev_group = arith.select(has_prev_group, group_idx - 1, fx.Index(0))
    ...
    def launch_hgemm_kernel(
        ...
        grid_x = total_tiles if GROUPED else bm * N_BLOCKS
        grid_y = 1 if GROUPED else SPLIT_K
