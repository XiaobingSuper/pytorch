...
bool mkldnnPrepackedConvIsSupported(
  ...
  if (!((input.dtype == c10::ScalarType::Float && weight.dtype == c10::ScalarType::Float) ||
      (input.dtype == c10::ScalarType::BFloat16 && weight.dtype == c10::ScalarType::BFloat16))) {
    GRAPH_DEBUG("mkldnnPrepackedConvIsSupported: only float32 and bfloat16 allowed");
    return false;
  }
  if (input.dims.size() != 4 || weight.dims.size() != 4) {
...
Tensor computeMkldnnPrepackedConvBinaryRun(
    const std::vector<ArgValue>& inputs,
    const std::vector<ExprHandle>& outputShape,
    const std::vector<ExprHandle>& outputStrides,
    const c10::optional<ScalarType>& outputType,
  ...
  BufHandle ResultBuf(
      "mkldnn_prepacked_conv_binary_run", outputShape, outputStrides, dtype);
  const BufHandle& inp = c10::get<BufHandle>(inputs[0]);
  const BufHandle& other = c10::get<BufHandle>(inputs[1]);
  const BufHandle& prepacked = c10::get<BufHandle>(inputs[2]);
  StmtPtr s = ExternalCall::make(
      ResultBuf,
      "nnc_mkldnn_prepacked_conv_binary_run",
      {inp, other, prepacked},
      {});
  return Tensor(ResultBuf.node(), s);
}
